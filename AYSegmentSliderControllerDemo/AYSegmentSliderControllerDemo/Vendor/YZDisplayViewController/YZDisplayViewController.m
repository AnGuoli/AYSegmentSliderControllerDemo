...
/** 所有小红点数组 */
@property (nonatomic, strong) NSMutableArray *redRounds;
...
- (NSMutableArray *)redRounds
{
    if (_redRounds == nil) {
        _redRounds = [NSMutableArray array];
    }
    return _redRounds;
}
...
#warning 加小圆点
        UIView *redRound = [[UIView alloc] initWithFrame:CGRectMake(labelW-5, 0, 5, 5)];
        redRound.backgroundColor = [UIColor redColor];
        [label addSubview:redRound];
        [self.redRounds addObject:redRound];
        redRound.hidden = YES;
...
- (void)controllRedRoundsHidden:(NSMutableArray *)indexArray {
    //    for (UIView *redRounds in self.redRounds) {
    //        redRounds.hidden = YES;
    //    }
    
    for (NSInteger i = 0; i < self.titleLabels.count; i++) {
        UIView *redRound = self.redRounds[i];
        ...
        NSString *hiddenIndexStr = indexArray.firstObject;
        NSInteger hiddenIndex = [hiddenIndexStr integerValue];
        
        if (hiddenIndex == i) {
            redRound.hidden = NO;
            [indexArray removeObject:hiddenIndexStr];
        } else {
            redRound.hidden = YES;
        }
    }
